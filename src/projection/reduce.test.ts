...
const content: ContentNode[] = [{ text: 'hello', type: 'text' }]

function msg(overrides: Partial<CanonicalMessageEvent> = {}): CanonicalMessageEvent {
	return {
		attachments: [],
		chatId: 'chat1',
		content,
...
function edit(overrides: Partial<CanonicalEditEvent> = {}): CanonicalEditEvent {
	return {
		attachments: [],
		chatId: 'chat1',
...
function del(overrides: Partial<CanonicalDeleteEvent> = {}): CanonicalDeleteEvent {
	return {
		chatId: 'chat1',
		messageIds: ['1'],
		receivedAtMs: 3000,
...
describe('reduce', () => {
	describe('message events', () => {
		...
		it('sets replyToMessageId and forwardInfo when present', () => {
			const ic = reduce(createEmptyIC('chat1'), msg({
				forwardInfo: { date: 100, senderName: 'Someone' },
				replyToMessageId: '99',
			}))

			const node = ic.nodes[0] as ICMessage
			expect(node.replyToMessageId).toBe('99')
			expect(node.forwardInfo).toEqual({ date: 100, senderName: 'Someone' })
		})
	...
	describe('metaReducer — user rename detection', () => {
		it('inserts ICUserRenamedEvent when displayName changes', () => {
			const renamedAlice: CanonicalUser = { displayName: 'Alice New', id: '1', isBot: false, username: 'alice' }
			let ic = reduce(createEmptyIC('chat1'), msg())
			ic = reduce(ic, msg({ messageId: '2', receivedAtMs: 2000, sender: renamedAlice, timestampSec: 2 }))

			expect(ic.nodes).toHaveLength(3)
			expect(ic.nodes[0]!.type).toBe('message')
			expect(ic.nodes[1]!.type).toBe('system_event')
			if (ic.nodes[1]!.type !== 'system_event')
				throw new Error('expected system_event')
			...
			if (sysEvent.type !== 'system_event')
				throw new Error('expected system_event')
			expect(sysEvent.kind).toBe('user_renamed')
			if (sysEvent.kind !== 'user_renamed')
				throw new Error('expected user_renamed')
		...
		it('inserts ICUserRenamedEvent when username changes', () => {
			const renamedAlice: CanonicalUser = { displayName: 'Alice', id: '1', isBot: false, username: 'alice_new' }
			let ic = reduce(createEmptyIC('chat1'), msg())
			ic = reduce(ic, msg({ messageId: '2', receivedAtMs: 2000, sender: renamedAlice, timestampSec: 2 }))
		...
		it('treats undefined and null username as equivalent', () => {
			const noUsername: CanonicalUser = { displayName: 'Alice', id: '1', isBot: false }
			const nullUsername: CanonicalUser = { displayName: 'Alice', id: '1', isBot: false, username: undefined }
			let ic = reduce(createEmptyIC('chat1'), msg({ sender: noUsername }))
			ic = reduce(ic, msg({ messageId: '2', receivedAtMs: 2000, sender: nullUsername, timestampSec: 2 }))
	...
	describe('edit events', () => {
		it('updates content, attachments, and sets editedAtSec on existing message', () => {
			...
			expect(ic.nodes).toHaveLength(1)
			const node = ic.nodes[0] as ICMessage
			expect(node.content).toEqual([{ text: 'edited', type: 'text' }])
	...
	describe('delete events', () => {
		...
		it('handles multiple messageIds', () => {
			let ic = reduce(createEmptyIC('chat1'), msg())
			ic = reduce(ic, msg({ messageId: '2', receivedAtMs: 2000, sender: bob, timestampSec: 2 }))
	...
	describe('service events', () => {
		...
		it('pushes ICMessageReactionEvent', () => {
			const ic = reduce(createEmptyIC('chat1'), service({
				action: { action: 'message_reaction', messageId: '1', operation: 'added', reaction: 'eyes' },
			}))
			const node = ic.nodes[0]!
