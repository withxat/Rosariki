...
export type SlackReplyPlacementMode = 'thread-default' | 'thread-required'
...
function latestMatching(segments: RenderedContextSegment[], pred: (seg: RenderedContextSegment) => boolean): RenderedContextSegment | undefined {
	let best: RenderedContextSegment | undefined
	for (const seg of segments) {
		if (!pred(seg))
			continue
		if (!best || seg.receivedAtMs > best.receivedAtMs)
			best = seg
	}
	return best
}
 ...
 */
export function computeSlackReplyPlacement(rc: RenderedContext, lastProcessedMs: number): SlackReplyPlacement | undefined {
...
export function renderSlackReplyPlacementXml(placement: SlackReplyPlacement): string {
	const lines = [
